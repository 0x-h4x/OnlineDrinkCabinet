...
If you deploy `index.html`, `styles.css`, and `app.js` to a static host while the Express API lives elsewhere, set the remote API origin on the `<body>` element:

```html
<body data-api-base="https://your-api-host.example.com">
```

The UI will forward all `/api/...` requests to that origin so the app keeps working even when the front end and backend are served from different locations.
